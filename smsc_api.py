...
setting_dir = os.path.abspath(os.path.join(os.path.dirname(__file__),
                              '..', '..'))
sys.path.append(setting_dir)

from setting import SMS_LOGIN, SMS_PASSWORD 

	
# Константы для настройки библиотеки
SMSC_LOGIN = SMS_LOGIN			# логин клиента
SMSC_PASSWORD = SMS_PASSWORD	# пароль
...
class SMSC(object):
	...
	def send_sms(self, phones, message, translit=1, time="", id=0, format=0, sender=False, query=""):
